...
export const getAllRestaurantsCurrentDayMenus = async () => {
  ...
  const currentDay = validWeekDays.includes(rawDay as WeekDay)
    ? (rawDay as WeekDay)
    : "Monday"; // fallback to Monday or handle error as needed
  const isoDate = new Date().toISOString();
  const weekDayIndex = WeekDayEnum[currentDay];
  const zonedIsoDate = toZonedTime(isoDate, TIME_ZONE);

  const intraMenusResponse =
    await fetchIntraCurrentDayMenuFromApi(zonedIsoDate);
  ...
  const restaurant: RestaurantMenus = {
    ...
    huoltamo: Array.isArray(intraMenusResponse.huoltamo)
      ? intraMenusResponse.huoltamo
      : [],
