...
interface PasilanVisioApiResponse {
  dayOfWeek: string;
  date: string;
  menuPackages: any[];
  html: string;
...
export const fetchVisioPasilaCurrentDayMenuFromApi = async (): Promise<MenuItems> => {
  try {
    const url = VISIO_PASILA_API_URL + `&date=${new Date().toISOString().slice(0, 10)}`;
